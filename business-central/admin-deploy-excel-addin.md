...
## <a name="known-limitations-in-business-logic"></a>Bekende beperkingen in bedrijfslogica

|Pagina  |Beperking| Opmerkingen  |
|-------|---------|---------|
|Verkooporders|Foutbericht: 'Microsoft Dynamics 365 Business Central Data Services heeft geprobeerd een client-callback uit te voeren om pagina 301 Ship-to Address List als modaal uit te voeren'. Client-callbacks worden niet ondersteund op Microsoft Dynamics 365 Business Central Data Services. | De **Verzendcode** op de **Verkooporder** pagina kan alleen worden bewerkt met specifieke Verzendopties. Als u  **Alternatief verzendadres** instelt op  **Verzenden naar**, wordt het modale dialoogvenster  **Lijst met verzendadressen**  geopend. Dit dialoogvenster is niet compatibel met Bewerken in Excel.|  
|Projectdagboek|Bijwerken van het veld  **Eenheidsprijs**  leidt niet tot een update van het  **Regelbedrag**. In plaats daarvan wordt  **Lijnkorting** bijgewerkt.| Met de webclient kunt u velden in elke gewenste volgorde bijwerken: prijs, bedrag, korting.&; Andere velden worden automatisch bijgewerkt. Om cascade-updates te voorkomen, beschikken de velden over geavanceerde logica die afhankelijk is van xRec. xRec gedraagt zich anders wanneer het via API's wordt aangeroepen.|

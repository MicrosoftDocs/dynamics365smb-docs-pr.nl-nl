---
title: 'Items verzamelen of verplaatsen voor productie, assemblage of projecten in basismagazijnconfiguraties'
...
---
# <a name="pick-for-production-assembly-or-projects-in-basic-warehouse-configurations"></a>Selectie voor productie, assemblage of projecten in basismagazijnconfiguraties

Hoe u componenten voor productie-, project- of assemblageorders selecteert, hangt af van de locatie waarop uw magazijn is ingericht. Zie voor meer informatie [Magazijnbeheer instellen](warehouse-setup-warehouse.md).

Activeer in een basismagazijnconfiguratie voor de uitgaande stroom (picken) op de pagina  **Locatie kaart**  voor de locatie de volgende instellingen:

* Productie, selecteer **Inventaris Pick/Verplaatsing** in het veld **Prod. Verbruik Magazijnverwerking** .
* Assemblage, selecteer **Inventarisverplaatsing** in het veld **Asm. Verbruik Magazijnverwerking** .
* Projectbeheer, selecteer **Inventory Pick** in het veld **project Consumption Warehouse Handling** .
...
* Wanneer u een inventarisatie registreert voor een interne bewerking, zoals productie of een project, wordt het verbruik van de verzamelde componenten tegelijkertijd geboekt.
...
Voor locaties waar bakken worden gebruikt, kunt u de stroom uitbreiden naar het project met  **Voorraadverplaatsingsdocumenten** .

> [!NOTE]
> [!INCLUDE[prod_short](includes/prod_short.md)] gebruikt de waarde in het veld **Resterend aantal** op de projectplanningsregel wanneer deze voorraadpicks maakt. Om voorraadselecties voor projecten te gebruiken, moet u de schakelaar  **gebruiksnummer koppelen toepassen** op de pagina  **projectnummer kaart** voor de projecten inschakelen. Hiermee kunt u het gebruik volgen ten opzichte van uw plan. Als u de schakelaar niet aanzet, blijft het resterende aantal op **0** en wordt de voorraadkeuze niet gemaakt. Zie voor meer informatie [Bijhouden van projectgebruik instellen](projects-how-setup-jobs.md?tabs=current-experience#to-set-up-project-usage-tracking).
...
1. Selecteer in het brondocument, dat een productieorder of project kan zijn, de actie  **Voorraadopslag/-picking maken** .  
...
1. Selecteer in het brondocument (dat een productieorder, assemblageorder of project kan zijn) de actie  **Voorraadopslag/-picking maken** .  
...
|Projecten | Wijzig de status van een project naar  **Open** of maak direct een project met de status Open.|  
